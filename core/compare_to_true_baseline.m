...
% NOTE: Sometimes during the first iterations the estimated density may be
% poor and very peaked. The density may then evaluate zero in each grid 
% point and, consequently, the TV and L2 become NaN.
...
%% Compute also estimated posterior mean and covariance matrix
if d > 2 && isfield(estim_post,'samples') && ~isempty(estim_post.samples)
    res.post_stats = basic_post_sample_stats(estim_post.samples,d);
end
end
